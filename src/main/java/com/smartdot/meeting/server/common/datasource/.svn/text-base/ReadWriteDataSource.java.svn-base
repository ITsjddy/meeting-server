package com.smartdot.meeting.server.common.datasource;
 ...
 * 目前实现功能 
 *   一写库多读库选择功能，请参考 
 *      @see com.smartdot.meeting.server.common.datasource.ReadWriteDataSourceDecision
        @see com.smartdot.meeting.server.common.datasource.DataSourceType 
 *    
 *   默认按顺序轮询使用读库 
 *   默认选择写库 
 *    
...
public class ReadWriteDataSource extends AbstractDataSource implements InitializingBean {  
    ...
    private DataSource determineReadDataSource() {  
    	...
    	if(null == readDataSources || readDataSources.length == 0){
    		log.debug("no read datasource exist,change choice to read/write 	"); 
    		return writeDataSource;
    	}
        //按照顺序选择读库   
        //TODO 读库负载均衡算法改进   
