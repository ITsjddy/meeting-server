...
guestApp.controller("guestController", function($scope,$http) {
	...
	$scope.guestSave = function(){
		$scope.guest.parentid= $('#parentid').val();
		var delurl = CONTEXT_PATH+DISPATCHER_PATH+"/department/updatedept";
		          	...
		          	$http({
						...
						params : param,
		                  	...
		                  	if(null != resultMap && resultMap.success == true){
		  						...
		  						parent.noty({dismissQueue: true, force: true, layout: 'topRight', theme: 'defaultTheme', text: '保存错误，您所修改的团存在子团！', type: 'error',timeout: 10000});
		  					}
		                  }).error(function(error) {
		                  	if('403' == error.status){
...
//			async:false,  angularjs 本身结构不支持异步请求
	...
	$scope.index = function(){
		window.location.href = CONTEXT_PATH+DISPATCHER_PATH+"/department/indexinclude?groupid="+$('#groupid').val();
	}
