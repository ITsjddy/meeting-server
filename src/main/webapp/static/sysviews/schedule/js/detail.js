...
scheduleApp.controller("scheduleController", function($scope,$http) {
	...
	$scope.getEntityById = function(){
		var url = LINK_PATH+"/schedule/getEntityById";
		var id = $("#id").val();
		var param = {id:id};
		$http({
			method : 'POST',
			...
			$scope.schedule = resultMap.schedule;
...
//			async:false,  angularjs 本身结构不支持异步请求
	...
	$scope.editSave = function(){
		var url = CONTEXT_PATH+"/"+DISPATCHER_PATH+"/schedule/editSave";
		...
		$http({
			method : 'POST',
			url : url,
			params :{scheduleMain : $scope.schedule, scheduleList : $scope.scheduleList},
			headers : {
				'Content-Type' : 'application/x-www-form-urlencoded;charset=utf-8'
			}
		}).success(function(resultMap) {
