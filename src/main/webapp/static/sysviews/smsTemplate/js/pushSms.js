...
/*初始化分页service*/
pageService(pushSmsPage); 

pushSmsPage.controller("pushSmsCtlr", function($scope, $http, pageService) {
  ...
  function creatWindowMember(type) {
    pageService.reGetDataForList($scope, $http, CONTEXT_PATH + "/" 
    		+ DISPATCHER_PATH + "/appUserForWindow/memberList?type=" 
    		+ $scope.member.id + "&name=" + $scope.member.name);
  }
