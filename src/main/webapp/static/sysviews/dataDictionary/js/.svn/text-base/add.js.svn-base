...
addDataDictionary.controller("spAddCtlr", function($scope, $http) {
  ...
  function initSavorPoint() {
	  ...
	  $http.get(urlStr + "findAllDataDictionary?dDTypeLogo=" + dDTypeLogo).success(function(response) {
		  $scope.dataDictionary = {dDType:typeName,dDTypeLogo:dDTypeLogo,dDNum:parseInt(response)};
	    });
	...
	return {
		...
		link : function(scope, elements, attrs, controller) {
			elements[0].onblur = function() {
				console.info(elements[0].id);
				var url = CONTEXT_PATH + DISPATCHER_PATH + "/dataDictionary/checkName";
				$http({
					method : 'POST',
					url : url,
					params : {dDName:this.value,languageType:elements[0].id,dDTypeLogo:$('#dDTypeLogo').val()},
		...
		link : function(scope, elements, attrs, controller) {
			elements[0].onblur = function() {
				...
				$http({
					method : 'POST',
					url : url,
					params : {dDLogo:this.value,dDTypeLogo:$('#dDTypeLogo').val()},
				}).success(function(resultMap) {
					if(resultMap == 'false'){
