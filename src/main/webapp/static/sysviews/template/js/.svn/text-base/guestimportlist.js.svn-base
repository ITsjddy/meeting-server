...
memberListApp.controller("memberListController", function($scope,$http,pageService) {
	...
	$scope.queryAllIds = function() {
		var url = CONTEXT_PATH+DISPATCHER_PATH+"/guesttemplate/queryAllIds";
		$http({
			method : 'POST',
			url : url,
			headers : {
	...
	$scope.queryAllIds();
	pageService.queryForList($scope,$http,CONTEXT_PATH+"/"+DISPATCHER_PATH+"/guesttemplate/pageQueryForCheck");
	$scope.searchByParam = function(){
		pageService.reGetDataForList($scope,$http,CONTEXT_PATH+"/"+DISPATCHER_PATH+"/guesttemplate/pageQueryForCheck");
	};
