...
guestApp.controller("guestController", function($scope,$http) {
	$scope.guestSave = function(){
		...
		var delurl = CONTEXT_PATH+DISPATCHER_PATH+"/guesttemplate/save";
	...
	$scope.createExcel = function(getArrIds){
		window.location.href = CONTEXT_PATH+DISPATCHER_PATH+"/guesttemplate/createlist?selectname="+getArrIds;
	}
	$scope.index = function(){
		window.location.href = CONTEXT_PATH+DISPATCHER_PATH+"/guesttemplate/index";
	}
	$scope.save = function(){
		var delurl = CONTEXT_PATH+DISPATCHER_PATH+"/guesttemplate/importExcelDept";
		var fd = new FormData();
