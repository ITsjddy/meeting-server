var adetcloudGroup = angular.module('adetcloudGroup', [ 'tm.pagination' ]);
...
adetcloudGroup.controller("adetcgCller", function($scope, $http, pageService) {
	...
	initCloudGroup();
	pageService.queryForList($scope, $http, CONTEXT_PATH + "/" + DISPATCHER_PATH 
			+ "/appUserForWindow/memberList?type=" + 'asdf');
	/**
	 * 全选按钮窗口
	 */
	...
	function initCloudGroup() {
		...
		$http.get(mainUrl + "queryById?id=" + spValue).success(
				function(response) {
