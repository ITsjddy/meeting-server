...
questionnaireApp.controller("questionnaireController", function($scope, $http, $log) {
	...
	$scope.fchat.combineTopics = function() {
		...
		for (var i = 0; i < $scope.fchat.topics.length; i++) {
			...
			if ($scope.fchat.options[i][0][0].content === undefined
					|| $scope.fchat.options[i][0][0].content === "") {
				continue;
			}
			...
			for (var j = 0; j < $scope.fchat.options[i].length; j++) {
				option += "," + $scope.fchat.options[i][j][0].content;
			}
	...
	$scope.questionnaireFindById = function() {
		var url = CONTEXT_PATH + "/" + DISPATCHER_PATH + "/questionnaire/questionnaireFindById";
		...
		$http({
			...
			$scope.questionnaire = resultMap;
			$scope.fchat.topics = resultMap.topics;
			...
			for (var i = 0; i < $scope.fchat.topics.length; i++) {
				var options = $scope.fchat.topics[i].options;
				
				$scope.fchat.options[i] = [ [ {content : ""} ] ];
				$scope.fchat.canDescOptions[i] = [ [ false ] ];
				for (var j = 0; j < options.length; j++) {
					$scope.fchat.options[i][j] = [{content : options[j].content}];
					if(options.length >1){
						$scope.fchat.canDescOptions[i][j] = [ [ true ] ];
					}
				}
			}
	...
	$scope.questionnaireFindById();
	
	$scope.questionnaireEditSave = function() {
		// 获取到表单是否验证通过
		if (!$scope.form.$valid) {
			alert('表单没有通过验证');
			return;
		...
		var url = CONTEXT_PATH + "/" + DISPATCHER_PATH
				+ "/questionnaire/questionnaireEditSave";
		var param = {
			id : id,
			title : title,
